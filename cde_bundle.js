(() => {
  ...
  var ACTIVE_MODULE = (() => { try { const q = new URLSearchParams(location.search); return (q.get("m") || q.get("module") || "hrm").toLowerCase(); } catch (e) { return "hrm"; } })();
...
"\u67E5\u8A62\u6458\u8981","\u532F\u51FA\u7BC4\u570D:${scope}","\u7B46\u6578:${res.ReturnData.RowCount}","\u532F\u51FA\u6642\u9593:${nowStr()}"`;
  ...
  async function init() {
    ...
    bindDropZone();
    setConfigSource("loading");
    CURRENT_FOLDER = folderForFunc(DEFAULT_FUNC);
    const ok = await tryLoadExternal(true, DEFAULT_FUNC, CURRENT_FOLDER);
    if (!ok) showNoConfigPrompt();
  }
